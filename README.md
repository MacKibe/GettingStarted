...
2.  First of all, you will have to download the latest version of the MySQL Community Server from the [official website](https://dev.mysql.com/downloads/mysql/)
3.  Select your Operating system Microsft Windows.
4.  In the Downloads section, select and download the ZIP file “Windows (x86, 64-bit), ZIP Archive"
...
9.  Download the mysql folder from [Mysql site](https://dev.mysql.com/downloads/mysql/) into C:\xampp
...
14. If you get an 'Mysql Command Not Found' error means the computer does not understand the mysql command so what you need to do is add mysql to windows environment.
...
15. Last thing is installing pma(phpmyadmin) to allow in creating of tables in your database.

- Ensure your MYSql service is running in Xampp.
- Click on admin button under MySql service
- The Phpmyadmin site will open on your browser.
- Click on the [import](http://localhost/phpmyadmin/index.php?route=/server/import) tab.
- In the (File to Import) section, choose the file you want to import.
- The file you are to import is _create table_ file location should be in C:\xampp\phpMyAdmin\sql
- Then click on import button, you should notice a new table named phpmyadmin was created and is listed in the databases side panel.
